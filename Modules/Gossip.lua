...
AQG:RegisterEvent("GOSSIP_SHOW", function()
    ...
    -- Debug mode: print options and what we would select, but don't act
    if db.debugEnabled then
        AQG:DebugSeparator("GOSSIP_SHOW (Gossip)")
        AQG:Debug("Gossip options (" .. #options .. "):")
        ...
        for i, option in ipairs(options) do
            ...
            AQG:Debug("  " .. i .. ". " .. (option.name or "?") .. " (ID: " .. tostring(id) .. ")" .. autoSelect .. nilTag)
        ...
        elseif wouldSelect then
            AQG:Debug("-> Would auto-select option " .. wouldSelect .. ": " .. (options[wouldSelect].name or "?"))
        else
            AQG:Debug("-> No valid option to auto-select.")
        end
        return
    end
